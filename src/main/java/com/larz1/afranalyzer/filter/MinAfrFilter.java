...
@Component
public class MinAfrFilter extends AbstractAfrFilter {

    @Autowired
    public MinAfrFilter(AfrAnalyzerSettings afrAnalyzerSettings) {
        super(afrAnalyzerSettings);
    }

    @Override
    public boolean filter(LogValue logValue, Object... args) {
        if (afrAnalyzerSettings.minAfrEnabled) {
            return logValue.getAfr() < afrAnalyzerSettings.minAfr;
        }
